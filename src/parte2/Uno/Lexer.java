...
public class Lexer {
	...
	public Token lexical_scan() {
		...
		default:
			if (Character.isLetter(peek)) {
				...
				// nonostante non preveda l'uso di keyword come if,else ecc. li
				// riconosce come identificatori
				// else if(this.javaIdentifier(s)){
				// Word w = new Word(Tag.ID, s);
				// words.put(s, w);
				// return w;
				// else{ System.err.println("invalid pattern identifier: " + s);
				// return null;}
				else {
					Word w = new Word(Tag.ID, s);
					words.put(s, w);
	...
	public static void main(String[] args) {
		...
		/*
		 * uso del file di input esterno Lexer lex = new Lexer(); String
		 * inputFileName = new File("").getAbsolutePath().concat(
		 * "\\src\\").concat(lex.getClass().getPackage().getName().replace('.', '\\').concat("
		 * \\Input.txt")); try { System.out.println(); BufferedReader br = new
		 * BufferedReader(new FileReader(inputFileName)); Token tok; do{ tok =
		 * lex.lexical_scan(br); System.out.println("Scan: " + tok.ToString());
		 * }while(tok.tag != Tag.EOF); br.close(); } catch (IOException e) {
